...
export default function DisclaimerForm({ disclaimer, errors }: DisclaimerFormProps) {
    const isEditMode = !!disclaimer?.id;
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Manajemen Disclaimer',
            href: route('disclaimer.index'),
        },
        {
            title: isEditMode ? 'Edit Disclaimer' : 'Tambah Disclaimer',
            href: isEditMode ? route('disclaimer.edit', disclaimer.id) : route('disclaimer.create'),
        },
    ];

    const { data, setData, post, processing } = useForm<Disclaimer>({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditMode) {
            post(route('disclaimer.update', disclaimer.id));
        } else {
            post(route('disclaimer.store'));
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEditMode ? 'Edit Disclaimer' : 'Tambah Disclaimer'} />

            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <h1 className="text-2xl font-bold">{isEditMode ? 'Edit Disclaimer' : 'Tambah Disclaimer'}</h1>
                    </div>
                </div>

                {errors && Object.keys(errors).some(key => !['disclaimer'].includes(key)) && (
                ...
                <div>
                    <Tabs defaultValue="content" className="w-full">
                        ...
                        <form id="disclaimer-form" onSubmit={handleSubmit} className="space-y-6 mt-6">
                                    ...
                                    <div className="mb-12">
                                        {mounted && (
                                            <QuillEditor
                                                ...
                                                onChange={(value) => setData('disclaimer', value)}
                                                placeholder="Masukkan Disclaimer"
                            ...
                            <div className="flex justify-end gap-2 pt-6">
                                <Button
                                    ...
                                    asChild
                                >
                                    <Link href={route('disclaimer.index')}>Batal</Link>
                                </Button>
