...
export default function TermsConditionForm({ termsCondition, errors }: TermsConditionFormProps) {
    const isEditMode = !!termsCondition?.id;
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Terms & Condition Management',
            href: route('terms.index'),
        },
        {
            title: isEditMode ? 'Edit Terms & Condition' : 'Tambah Terms & Condition',
            href: isEditMode ? route('terms.edit', termsCondition.id) : route('terms.create'),
        },
    ];

    const { data, setData, post, processing } = useForm<TermsCondition>({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditMode) {
            post(route('terms.update', termsCondition.id));
        } else {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEditMode ? 'Edit Terms & Condition' : 'Tambah Terms & Condition'} />

            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <h1 className="text-2xl font-bold">{isEditMode ? 'Edit Terms & Condition' : 'Tambah Terms & Condition'}</h1>
                    </div>
                </div>

                {errors && Object.keys(errors).some(key => !['terms_and_condition'].includes(key)) && (
                ...
                <div>
                    <Tabs defaultValue="content" className="w-full">
                        ...
                        <form id="terms-form" onSubmit={handleSubmit} className="space-y-6 mt-6">
                                    ...
                                    <div className="mb-12">
                                        {mounted && (
                                            <QuillEditor
                                                ...
                                                onChange={(value) => setData('terms_and_condition', value)}
                            ...
                            <div className="flex justify-end gap-2 pt-6">
                                <Button
                                    ...
                                    asChild
                                >
                                    <Link href={route('terms.index')}>Batal</Link>
                                </Button>
